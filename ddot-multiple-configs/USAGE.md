...
### Use Case 1: Dual Shipping (This Repo)

**Goal**: Send telemetry to both Datadog and another backend

**Solution**:
- Base config: Standard DDOT
- Override config: Add OTLP exporter and modify pipelines

**Result**: All telemetry goes to both destinations

### Use Case 2: Adding Debug Logging

**Goal**: Temporarily enable detailed logging for troubleshooting

**Solution**:
```yaml
# otel-config-debug.yaml
exporters:
  debug:
    verbosity: detailed
...
Add this config to items, restart, debug, then remove.

### Use Case 3: Custom Sampling

**Goal**: Reduce data volume in high-traffic services

**Solution**:
```yaml
# otel-config-sampling.yaml
processors:
  probabilistic_sampler:
    sampling_percentage: 10
...
### Use Case 4: Legacy Protocol Support

**Goal**: Accept Jaeger traces from legacy services

**Solution**:
```yaml
# otel-config-jaeger.yaml
receivers:
  jaeger:
    protocols:
      grpc:
